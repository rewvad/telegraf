...
type Jenkins struct {
        ...
        LableInclude        []string          `toml:"lable_include"`
...
func (j *Jenkins) gatherLabelData(n *labelResponse, acc telegraf.Accumulator) error {

        tags := map[string]string{}
        if n.Name == "" {
                return fmt.Errorf("error empty lable name")
        }

        tags["lable_name"] = n.Name

        fields := map[string]interface{}{
                "total_executors":    n.TotalExecutors,
                "idle_executors":   n.IdleExecutors,
                "busy_executors":    n.BusyExecutors,
        }
...
func (j *Jenkins) gatherLabels(acc telegraf.Accumulator) {
        var wg sync.WaitGroup
        for _, lable := range j.LableInclude {
                labelResp, err := j.client.getLable(context.Background(), lable)
                if err != nil {
			fmt.Println("Errors gets")
                        acc.AddError(err)
                        return
                }
                err = j.gatherLabelData(labelResp, acc)
                if err == nil {
                        continue
                }
                acc.AddError(err)
